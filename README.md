<div align="center">
  <h1>Hi, I'm 卷娄 / 赵康</h1>

  <p>
    <strong>前端工程师 · AI 工程学习者 · 长期折腾自己的表达和方法论</strong>
  </p>

  <p>
    <em>知其卷娄之患，故守其不卷不娄之身。</em>
  </p>

  <p>
    <a href="https://juanlou.top/">
      <img src="https://img.shields.io/badge/Site-juanlou.top-0f766e?style=flat-square" alt="Site" />
    </a>
    <a href="mailto:juanlou.zhao@gmail.com">
      <img src="https://img.shields.io/badge/Email-juanlou.zhao%40gmail.com-2563eb?style=flat-square" alt="Email" />
    </a>
    <img src="https://img.shields.io/badge/WeChat-zk3106444523-16a34a?style=flat-square&logo=wechat&logoColor=white" alt="WeChat: zk3106444523" />
    <a href="https://github.com/juanlou1217">
      <img src="https://img.shields.io/badge/GitHub-juanlou1217-111827?style=flat-square&logo=github" alt="GitHub" />
    </a>
  </p>
</div>

<!-- profile-readme-refresh: 2026-06-28T15:52:00+0800 -->

---

### 关于我

我现在主要在做前端开发，也在持续学习 AI 工程、Agent 工作流、知识库和项目文档化。

比起只把一个需求做完，我更在意一件事背后的链路：为什么要做、它在业务里解决什么问题、代码结构会不会变成长期负担、能不能沉淀成以后还可以复用的经验。

我的博客叫 **卷娄的小屋**。那里会写技术，也会写生活、实习、焦虑、表达和成长。它不是一个只展示结果的地方，更像是我把模糊想法慢慢讲清楚的过程记录。

### 最近在关注

- 用 `React`、`TypeScript`、`Next.js` 做更稳定、更好维护的前端工程。
- 在 AI 产品和浏览器插件里，把复杂表单、用户输入和自动化流程拆得更清楚。
- 学习 `Agent`、`Workflow`、`Context Engineering`，理解 AI 系统什么时候该简单，什么时候值得复杂。
- 把项目知识整理成结构、约束和验证，让 AI coding agent 能更可靠地参与工程。

### 技术栈

<p>
  <img src="https://img.shields.io/badge/React-20232a?style=flat-square&logo=react&logoColor=61dafb" alt="React" />
  <img src="https://img.shields.io/badge/Next.js-111827?style=flat-square&logo=nextdotjs&logoColor=white" alt="Next.js" />
  <img src="https://img.shields.io/badge/TypeScript-2563eb?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-0891b2?style=flat-square&logo=tailwindcss&logoColor=white" alt="Tailwind CSS" />
  <img src="https://img.shields.io/badge/Node.js-166534?style=flat-square&logo=nodedotjs&logoColor=white" alt="Node.js" />
  <img src="https://img.shields.io/badge/Prisma-1f2937?style=flat-square&logo=prisma&logoColor=white" alt="Prisma" />
  <img src="https://img.shields.io/badge/PostgreSQL-1d4ed8?style=flat-square&logo=postgresql&logoColor=white" alt="PostgreSQL" />
  <img src="https://img.shields.io/badge/Rust-b45309?style=flat-square&logo=rust&logoColor=white" alt="Rust" />
</p>

### 我写过的一些东西

- [Building Effective Agents：从最小智能单元到 Agent 工程边界](https://juanlou-dev.vercel.app/blog/building-effective-agents)
- [Effective Context Engineering for AI Agents](https://juanlou-dev.vercel.app/blog/effective-context-engineering-for-ai-agents)
- [AI 工具使用的一些反思：从知识到结构，再到约束](https://juanlou-dev.vercel.app/blog/ai-workflow-from-knowledge-to-constraints)
- [最近的一些反思](https://juanlou-dev.vercel.app/blog/not-just-a-passive-executor)

---

<div align="center">
  <p>
    写代码，也写自己如何思考、探索和慢慢变清楚。
  </p>

  <p>
    <a href="https://juanlou-dev.vercel.app/">Site</a>
    ·
    <a href="mailto:juanlou.zhao@gmail.com">Email</a>
    ·
    WeChat: <code>zk3106444523</code>
    ·
    <a href="https://github.com/juanlou1217">GitHub</a>
  </p>
</div>
